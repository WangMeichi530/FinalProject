          ...
          <el-table-column label="showType" min-width="12%">
            <template slot-scope="scope">
              <el-select v-model="scope.row.htmlType">
                <el-option label="Text框" value="input" />
                <el-option label="Text域" value="textarea" />
                ...
                <el-option label="富Text控件" value="editor" />
              </el-select>
            </template>
          </el-table-column>
    ...
    submitForm() {
      ...
      Promise.all([basicForm, genForm].map(this.getFormPromise)).then(res => {
        ...
        if (validateResult) {
          ...
          this.$modal.msgError("表单校验未通过，请重New检查Submitinformation");
        }
      });
    },
    getFormPromise(form) {
